...
describe('#addCreditCard', () => {
  it('adds creditCard to unlocked vault', async () => {
    const type: ItemType = 'creditCard'
    await client.register('P@ssw0rd!')
    const vault = await client.createVault('')
    expect(vault.id).toBeTruthy()
    expect(client.isLocked()).toBe(false)

    const { id: itemId } = await client.addCreditCard(vault.id, {
      cardName: 'cardName',
    ...
    await expect(
      client.getItemSecret(vault.id, itemId, SecureFieldName.CardNumber),
    ).resolves.toEqual('cardNumber')
    await expect(
      client.getItemSecret(vault.id, itemId, SecureFieldName.CardSecurityCode),
    ).resolves.toEqual('cardSecurityCode')
  })
